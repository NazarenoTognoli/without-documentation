...
document.addEventListener("keydown", function(event) {
	if (event.key === 'Enter') {
		...
		case "generate.subTech": 
			if (generateSubTechConsole === true) {
				allConsoleFunc(false);
				conGenSubTech();
			}
			break;
		...
		//subTech/swtNum/ID
		case "generate.link":
			if (generateLinkConsole === true) {
				allConsoleFunc(false);
				conGenLink();
			}
			break;
		case "remove.link":
			if (removeLinkConsole === true) {
				allConsoleFunc(false);
				conRemLink();
			}
			break;
		...
		case "rlm":
			if (generateRlmConsole === true) {
				allConsoleFunc(false);
				conGenRLM();
			}
			break;
		case "remove.rlm":
			if (removeRlmConsole === true) {
				allConsoleFunc(false);
				conRemRLM();
			}
			break;
